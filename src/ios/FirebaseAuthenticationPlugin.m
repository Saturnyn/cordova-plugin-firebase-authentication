...
@interface FirebaseAuthenticationPlugin() {
	...
	CDVInvokedUrlCommand* signInWithAppleCommand;
}
   ...
   didCompleteWithAuthorization:(ASAuthorization *)authorization API_AVAILABLE(ios(13.0)) {
	if ([authorization.credential isKindOfClass:[ASAuthorizationAppleIDCredential class]]) {
		...
		[[FIRAuth auth] signInWithCredential:credential
			...
			if (error != nil) {
				NSLog(@"Sign in with Apple success: %@", authResult);
			} else {
				NSLog(@"Sign in with Apple received error: %@", error);
			}
			CDVPluginResult *pluginResult = [self createAuthResult:authResult withError:error];
